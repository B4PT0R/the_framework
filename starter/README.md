...
A small, editable desktop application built with `harness_core`: one persistent
...
From an environment with `harness_core` installed, run `harness-core bootstrap`
(or `python -m harness_core bootstrap`). Enter an empty directory for editable
...
do not add product imports to `harness_core`. Backend contributions use the
