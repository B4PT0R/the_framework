...
- `voice.py`: call-scoped voice lifecycle over the core RealtimeController.
...
| --- | --- |
| Neutral, independent assembly | `test_starter_assembles_with_product_imports_forbidden` builds with all product imports rejected; framework import/wheel tests independently check core distribution |
| Standalone bootstrap | `harness-core bootstrap` copies the wheel-bundled template into a separate code directory, records a private data directory, and refuses nonempty or overlapping targets; wheel-only and clean npm install/build tests verify extraction |
...
| Tool activity and interruption | Tool feedback events displayed in the composer; actual Bash roundtrip emits tool start/end; authenticated interrupt submission and core worker cancellation tests |
...
| Concise, pedagogical surface | One short composition, capability-owned modules, settings separated from chat, core service/control/upload reuse; no forwarding-only compatibility layers |
| Documentation and handoff | Starter README covers installation, explicit login, data layout, settings, extensions, self-modification, tests and external boundaries; root/core docs link to it |
...
- Scheduler/system service implementations and application controls are reusable
  core capabilities, consumed by both product and starter.
