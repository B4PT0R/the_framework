...
def test_wheel_contains_standalone_framework_and_prompt_resources(tmp_path, monkeypatch):
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        assert "harness_core/py.typed" in archive.namelist()
        assert "harness_core/utils/ids.py" in archive.namelist()
        assert "harness_core/utils/persistence.py" in archive.namelist()
        assert "harness_core/utils/tokens.py" in archive.namelist()
        assert "harness_core/ids.py" not in archive.namelist()
        assert "harness_core/persistence.py" not in archive.namelist()
        assert "harness_core/tokens.py" not in archive.namelist()
        for member in archive.namelist():
            if member.startswith("harness_core/"):
    ...
    subprocess.run(
        [sys.executable, "-c", (
            "from harness_core import AgentApplication, AgentSpec, ClientSurface, "
    ...
    subprocess.run(
        [sys.executable, "-m", "harness_core.agent.runtime.worker_process", "--help"],
        check=True, capture_output=True, text=True, timeout=30,
    )
    code = tmp_path / "my-agent"
    data = tmp_path / "my-agent-data"
    subprocess.run(
        [sys.executable, "-m", "harness_core", "bootstrap", "--code-dir", str(code), "--data-dir", str(data)],
        check=True, capture_output=True, text=True, timeout=30,
    )
